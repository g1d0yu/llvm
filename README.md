...
然而原论文的定理4，人家说的是u *→ v，然后v和w在图上有一条边，也就是完全脱离了半支配路径，直接是树边和非树边，我们上面的证明只能用在半支配路径里面。如何增强到原论文这个结论呢？其实树路径u *→ v，和u是最小的节点本质上是一样的。可以这样理解，如果u不是最小的，有个前面的节点比它还要小，那么假设这个更小的节点是m，由于引理1，m＜u，m到u要经过一个树上的公共祖先g节点，即 m =V0, V1 ... g .. Vk = u ，而g节点是u在树上的祖先，就有一条树路径g *→ u，那么g才是最小的节点，就令g是新的u。以此类推，u必定是整个路径上最小的节点，那么使用上面的证明思路，立即可得sd2(w) = sd(u)，这就是定理4全部的内涵——树路径本质上就是最小的节点。
