...
在认识一个新事物时，一上来就去看很多相关的教程。学了很久，走了很多弯路，愈发觉得自己学习上的低效。但是学习中一旦略有收获，也每每惊叹，在问题的解决过程中，总是蕴含着顶级的思维能力，，应该多去学思维。
...
还第二步是把可以提升的进行提升，这一步难理解一点，需要对图论有一定的认识。为什么呢？因为我们看到这个SSA形式的ir中出现了一个phi指令，phi指令应该插入到哪里？数学上证明了是一个支配边界的地方，而求支配边界的操作又涉及到了支配树、半支配节点、DFS生成树等概念。
...
把基本块标上序号，可以发现要执行块3必定先执行(必经)块2，执行块7必定先执行块2、3。从图论的角度，也就是节点2严格支配节点3；节点2、3严格支配节点7。节点5、6都是7的前驱节点，但不是7的严格支配节点，那么节点5和6的支配边界就都是节点7,。

执行基本块7必定先经过块3，而执行块3又必定经过块2，很容易发现如果节点A支配节点B，则被节点B支配的必定被节点A支配。图中离节点7最近的支配节点是3，因此节点3是节点7的直接支配节点。从这个角度，就很容易看明白支配树是什么了，自己学习只要方便自己理解即可。理解了支配树，再看一个例子巩固一下，测试一下是不是真的理解了。

![image-20250707154737563](./assets/image-20250707154737563.png)

从图的角度，怎么容易理解支配边界呢，我们可以看到基本块2和基本块7，都有一个特点，就是有不止一条的进入的路径，那么说明它们更可能是一个合流基本块，合流的基本块前驱中都有可能会对该基本块使用的变量进行赋值，因此更有可能插入φ函数

![image-20250707152508887](./assets/image-20250707152508887.png)

那么是不是所有的汇入(合流)节点都要插入φ函数呢？其实是不用的，比如这种情况，a和b的值其实是相同的，如果插入φ反而造成性能浪费。究竟哪些合流块要插入φ函数？支配边界。

![image-20250707153554528](./assets/image-20250707153554528.png)

支配边界可以高效插入phi指令，而支配边界可以由直接支配节点快速计算，当然这个计算也不好理解。直接支配节点又和支配树有关系，因此mem2reg中会涉及到求CFG的支配树。支配树这部分相当难，之前的方法是SLT算法，现在用的是另一种更容易理解的算法。理解证明很困难，但是理解概念很容易。后面我们只要理解简单的概念就行了，没必要死磕证明。

那如果能知道每个节点的直接支配节点，也可以马上构建一个支配树，这一点毫无疑问。这里我在看了很多求支配树相关的文章后发现，理解概念还是容易的比如“半支配节点”以及通过半支配计算直接支配节点，但是想知道为什么，也就是理解证明却很难。下面的几篇文章都很好，第一篇清晰明了的介绍了概念和应用，跟着看一遍大概能知道这个计算过程，很通俗易懂，能帮助理解。第2篇主要是几个定理的证明，第3篇是对论文的翻译。
...
这里斗胆结合几篇文章说明一下这个SLT求支配树的算法是怎么回事，CFG就用第一篇文章中的这一张，虽然有节点重复但是不影响理解，已经标出了DFS生成树和直接支配节点。仔细看图，我们发现如果一些节点只有一条入边，那就能马上得到直接支配节点，比如C、B、G、L、J等，那么相当于idom能直接知道几个

![image-20250709100047667](./assets/image-20250709100047667.png)

然后继续看概念，树边、非树边等。然后是祖先、真(完全)祖先这俩都是对于图G的DFS生成树T来说的，然后是横叉边(横跨边)、返祖边
...
引理4是关键的一个引理，我们通过论文对引理4来理解，论文中说存在一个DFS生成树上的路径r *→idom(w) ∗→sdom(w)+→w，这个很难理解，前面我们证明了idom(w)到w在T上必有路径，为什么sdom(w)到w也必有路径呢？

文章的这句话“**parent(w)是sdom-candidate**”，也就是说DFS生成树T上的某个节点w，其父节点parent(w)是w的候选半支配节点，这个是根据定义。并且sdom(w)≤parent(w)＜w，后面parent(w)＜w这个是肯定的，父节点必定小于子节点。前面这个sdom(w)≤parent(w)是为什么呢？

sdom定义就是所有半支配候选节点中最小的。假设sdom(w) > parent(w)，那么sdom不是候选半支配节点中最小的，和定义就矛盾了，这里其实意思就是sdom要尽量小。

又根据引理1，sdom(w)到w的所有路径上必定经过一个共同祖先v；根据定义sdom(w)一定是半支配路径的起点，那么就考虑任意一条半支配路径，sdom(w)到w的半支配路径必经共同祖先v，

这个共同祖先v有可能在半支配路径内部吗？那么根据半支配路径的定义，任何内部节点都要大于w，而v是祖先应该小于w，因此v不可能在半支配路径的内部。上面是论文的思路，但是后面论文中直接认为sdom是共同祖先，这个我们得假装不知道这回事，慢慢理解为什么

![image-20250707195032537](./assets/image-20250707195032537.png)

根据引理1，我们只知道sdom(w)到w的所有路径都必须经过这个共同祖先，所有的路径都是sdom(w) → ... → u → ... → w，因为u是必经节点。然而祖先u < w，因为sdom的定义就是内部都大于w，显然这个不是半支配路径，那sdom不可能是候选半支配节点，这就和sdom定义矛盾了，这就是论文中说的u不能在内部。因此，共同祖先u一定是sdom本身。

从这里，我们才能感觉到sdom，半支配节点是想做什么，也就是想在生成树T上找一个最早可能对w节点有影响的节点，通过定义本身入手去把不在树上的情况排除掉。我们可以想一下，横叉边必定是右树，就比parent(w)大，就不可能是sdom了，再去找横叉边的父节点，如果又大于说明还在右树上，通过这种递归的思想，逐渐逼近到树上w的祖先节点，这就是半支配节点定义的高明之处。通过下面这幅图就容易理解了，半支配路径内部就是右树，右树和w没有什么关联，但是右树的父节点，一定也是左树的父节点，相当于找到了左树内部w在树上的祖先，祖先是有可能的支配节点，因此叫半支配。这个定义的引入就是为了解决问题，这就是工程中顶级的思维能力。

![image-20250708102723468](./assets/image-20250708102723468.png)

再看idom的问题，第三篇文章中写的比较详细

![image-20250708103512699](./assets/image-20250708103512699.png)

那么引理4就完整的说明了idom、sdom的关系，就为工程实践铺好了道路。自己简单证一证，虽然可能不严谨。但是没有办法，哪怕只有一句感觉似是而非的地方，都说明自己还没有彻底理解。如果不搞清楚困惑，后面某些步骤就会被小小的困惑所桎梏，当困惑累积的越来越多，学习也难以为继。相反，如果我们用自己的方式理解了困惑，这会成为一种动力。

#### sdom的计算

前面的过程我们只是知道了sdom是什么，sdom有什么性质，那关键是怎么更方便地计算出来sdom呢？

论文中给出了一种方法，sdom只可能有两种情况，一个是不含内部节点的路径，也就是w节点的前驱节点，记作sd1；另一种是有内部节点的半支配路径，记作sd2。sd1肯定是存在的，最少有parent(w)，而sd2不一定存在，先记为无穷，那么sdom肯定是min(sd1，sd2)

![image-20250708111942600](./assets/image-20250708111942600.png)

sd1的情况很好算，只要遍历所有前驱就能得到最小的那一个。sd2的情况，论文中认为是最小的sd(u)，直观上我们能够感觉到u很像是一个半支配路径

![image-20250708135148083](./assets/image-20250708135148083.png)

然后考虑了一个半支配路径sd2(w)→w，证明sd2(w)=sd(u)，如果证明了那就相当于：求存在内部节点的半支配路径上的sd2(w)转化成了求最小的内部节点u的sdom(u)，而sdom(u)是唯一的，就能在代码上实现求解

![image-20250708135559740](./assets/image-20250708135559740.png)

看一下下图作者怎么证明，第一步这个是根据定义。人家定义的就是u是这条路径中最小的，那p肯定大于u，没问题
...
再看后面，在有向图G上面sd(u)到u的路径中，内部节点都大于u，毫无疑问，这就是半支配路径的定义，不大于就不能成为sdom了，又因为u本身大于w，这个是自带的条件，所以内部节点都大于w，没问题。于是，G上sd(u)→u，然后u到w的这条路径上，所有内部节点都大于w了，又满足半支配路径的定义，因此sd2(w)小于等于sd(u)，这个是为什么相信不需要再重复证明一次了。

最后已知sd2(w)≤sd(u)，而sd(u)≤sd2(w)，可以发现sdom(u)一定是等于sd2(w)
...
我们自己得清楚这个事：怎么证明一定存在这么一个路径呢？也就是凭什么说存在r到v不经过idom？还是反证，假设不存在不经过idom(w)，那么idom(w)应该是v的直接支配节点，与前面的条件(idom(v) +→idom(w)矛盾了，因此必定存在一条r到v路径不经过idom(w)，取这一条路径，构造出G上r到v再到w，不经过idom(w)，那么就与定义又矛盾了。这就是下面这个证明的内涵
...
然后解释论文又独立提出来一个引理4，这个引理，虽然没有标出来是原论文的定理2但是我们得知道这回事
...
论文的证明是怎么回事呢？论文中是sdom(w)+→u *→ w，这条路径一定注意，是树上的路径并且是存在的，因为前面的引理已经证明了sdom(w)到w在生成树上存在路径，然后u是生成树路径上的一个内部节点，并且这个u节点的sdom(u)≥sdom(w)，树上路径和图上路径必须要区别好。
...
然后又要找一个y节点，这个y节点必须是满足sdom(w) *→ y *→ w的第一个节点，这个路径是全部在树上的路径，这就意味着y ≥ sdom(w)

然后研究x到y的图上路径q=(x,v1,v2,……vk-1,y)，q必定是路径p(G上r-w)的一部分。那么在q上所有的节点都是大于y的，这个是为什么呢？我们乍一想好像不能直接断定这回事，还是那句话，如果对某句话似是而非，说明没有理解，就应该自己证明一下。

假设任意一个中间结点vᵢ小于y，vᵢ到y会必经一个树上祖先节点Vj，引理1得出类似vᵢ ...Vj... y ...w，这只是一个普通的图路径。论文中为什么直接得出了一个树上的路径sdom(w)*→vj *→ y→w，这条路径怎么来的？
...
`从第一次看到证明中“中间结点都大于y”，我就顿觉困惑，想不明白。找资料找教程都是一笔略过，问ai也是五花八门的回答。我想了将近一天，发现不应该纠结于图路径，而是要切换视角到生成树上。虽然想清楚一句话确实没什么用，还耗费了很多时间，不过一旦靠自己理解了某个知识，那喜悦之情也是难以言表的。`

证明上面有什么用呢？路径q，内部节点都大于y，那q就是一个半支配路径，立即得到sdom(y) ≤ x； 就又可以得出sdom(y) ≤ x < sdom(w).。

我们通过前面一大段内容推导出sdom(w) *→ y *→ w树路径上，离sdom(w)最近的这个y的**sdom(y) < sdom(w)**，把sdom(y) < sdom(w)记为推论①。然而再看一遍定理2“在sdom(w) +→ y *→ w上，**sdom(y)≥sdom(w)**”，定理2这个路径是sdom(w) +→ y *→ w，我们证明构造的路径是sdom(w) *→ y *→ w，因此我们的推论①在sdom(w) +→ y *→ w这一段与定理2的条件矛盾，因此y就不能在sdom(w) +→ y *→ w上，即y≥sdom(w)不能取到大于号，y又是第一个节点，那满足上面的条件情况下只能y=sdom(w)。由于y是任意路径p上的节点，只是对y加了大小限制，sdom(w)就支配w了

![image-20250710141758914](./assets/image-20250710141758914.png)

那么在一个特殊的条件下，idom(w)=sdom，但是更多的情况下，sdom和idom又有什么关系？这就是下面的定理3
...
然后我们还要证明一个引理，当然这个结论其实不证自明，想一想支配点和直接支配点的区别。利用这个结论，我们就得到了定理2的这个增强结论sdom(w) *→ idom(w)

![image-20250710195044432](./assets/image-20250710195044432.png)

此外，根据前面的证明揭示了一个道理：就是sdom(w)到w的树路径中，中间结点的sdom是小于sdom(w)的，这个证明就是定理2中这一大段；而中间节点v如果取到了w，就是sdom(v)≤sdom(w)，再令sdom(u)是sdom(v)中最小的，那么就有sdom(u)≤sdom(v)≤sdom(w)。那么我们再考虑一件事sdom(v)到v是存在一个树上路径的，而sdom(w) +→ v *→ w说明了sdom(w)是v在树上的祖先，那么sdom(v)要到v就要先经过这个祖先sd(v) ∗ → sd(w) + → v ∗ → w.，这个就是解释论文中构造的这条路径的意义

![image-20250711112643984](./assets/image-20250711112643984.png)

证明，由于这个定理3比较困难，我们把解释论文的证明思路都过一遍
