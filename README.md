...
在认识一个新事物时，看了很多教程，花了很久时间，走了很长弯路，愈发觉得自己学习上的低效。但是学习中一有收获，又有一种"遥襟俯畅，逸兴遄飞"之感。

遇到不懂的地方，自己总是在思考为什么，总想去弄懂。但是回过头来想一想，在问题的解决过程中，总是蕴含着前人顶级的思维能力，弄懂为什么就是去求知，有时候对于一个普通人来说太有求知欲不是一件好事，求索复杂的问题，往往消耗大量的时间也一无所获。

中学大学时代，我们做题时使用公式定理，并不会关心做题工具本身怎么证明，只是能用来解题就够了。有时候工作、学习、生活中也许也应该这样，太执着于原因和急于渴求满足感，说不定会让付出适得其反，不求甚解也是一种智慧吧。
...
只需要在支配边界插入phi指令就行了，而支配边界可以由直接支配节点快速计算。

## 6.1支配关系

所以我们研究怎么计算节点的直接支配节点。我在看了很多求支配树相关的文章后发现一个问题：很多文章不够严谨。应该直接去读原始论文，论文作者的写作思路明确由浅入深，层层递进。虽然证明有点难度，但是当我们从头开始做到能理解每一句话，那理解所有证明也并非遥不可及。不过论文是英文的，看着费力一些。下面的几篇文章也可以看，第一篇清晰明了的介绍了概念和应用，第2篇主要是几个定理的证明，第3篇是对一篇论文的解读。
...
我先把第一篇文章看了一遍，大概知道概念了。但是总感觉理解不了，仿佛死记硬背。于是就去看了论文，这里斗胆结合几篇文章说明支配树或者llvm中的求DT部分，大部分图都是别人文章的，证明内容有些我自己的思考。CFG就用论文中的这一张，文章1中虽然把节点画重复了，但是已经标出了DFS生成树和直接支配节点，不影响理解。仔细看图，我们发现如果一些节点只有一条入边，那就能马上得到直接支配节点，比如C、B、G、L、J等，那么相当于idom能直接知道几个，还是很有用的。

![image-20250709100047667](./assets/image-20250709100047667.png)

然后是概念

![image-20250718111015657](./assets/image-20250718111015657.png)

读完第一篇文章后，着重把握住DFS生成树相关的概念，比如祖先、真(完全)祖先。节点间的路径可能是生成树上的路径，也可能只是图G上的路径。我在后面发现还有些概念，比如前向边、横叉边这些其实没这么重要。重要的是搞清楚定理中某条路径是在说有向图G，还是在说生成树T，这能帮助我们把图上的路径问题转到树上路径或者半支配路径。此外，论文基本都是对的，很多话看不懂也先不要质疑正确性，这是因为作者的思路有较大跳跃，这源于他们的经验和积累，熟悉于视角的切换。

知道这些概念后，就可以开始证明了，第二篇文章中引理1、3没有给证明，对于作者确实太过简单无需证明。但是我们自己没证明一次就会容易忘记，尤其是引理1，是基础中的基础，后面多次要用到，万分重要。

![image-20250718111458030](./assets/image-20250718111458030.png)

这里我们用自己能理解的步骤证明。关键是自己能理解，而非过于学术性的证明。同时随着证明，我们对支配关系的理解也会加深，因此证明也会从详细到简略。
...
**情况1：v = w** 如果v = w，那么从v到w的路径就是自己到自己的路径，显然经过它们的共同祖先(节点本身)。这个相等的情况很容易理解

**情况2：v < w** 由于v < w，在DFS过程中v比w更早被访问。根据DFS的性质，当我们访问到w时，必然是通过某个已经访问过的节点到达的。考虑从v到w的任意路径P = v → u₁ → u₂ → ... → uₖ → w 我们需要证明这条路径必须经过v和w的某个共同祖先。并设lca(v,w)为v和w在DFS树上的共同祖先。
...
- v不是w的祖先，那一定不存在v→w且中间节点递增的这种路径。由于v < w，也就意味着当DFS访问完v的整个子树后，要经过回溯才会访问到w。如果有递增的，只能在一个子DFS树上

  ![image-20250718112227891](./assets/image-20250718112227891.png)

- v和w在DFS树中处于不同的子树，必须先回溯到一个较小的节点，DFS就是回溯到祖先节点lca(v)

- lca(v)也必须能到达w所在的子树，那w必定也是在该祖先节点lca(v)的子树上(lca(v)能递增访问到)

- 于是路径经过lca(v,w)

我们可得注意这个引理1多么的重要，这个是生成树上的公共祖先，那说明lca(v,w) *→v且lca(v,w) *→w这两条树路径是存在的，树路径节点大小都是能比较的。
...
反证法：假设idom(w)不是w在DFS树上的祖先。那么在DFS树中，从入口节点到w的树路径就不经过idom(w)，这与idom(w)支配w矛盾。因此，idom(w)必须是w在DFS树上的祖先。
...
解释论文中先介绍了候选半支配节点，候选这个词是很多文章中没出现过的，它大大简化了树边、非树边这些描述。候选半支配节点就是任意路径p(v=v0,v1,…,vk-1,vk=v)所有内部节点都大于w，这种路径就叫做半支配路径，起点v就叫做候选半支配节点。所有w的候选半支配节点中dfs序号最小的那个就是半支配节点sdom(w)。

论文中先介绍了候选半支配节点，任何G中的w节点的树边前驱(DFS生成树上的前驱)都是候选半支配节点；然后如果存在半支配路径，半支配路径就是有向图G上v→w，所有内部节点dfs序号都大于w，半支配的起点v也是一个半支配节点候选者。

所有候选者里面dfs序号最小的那个就是半支配节点。

![image-20250707200638338](./assets/image-20250707200638338.png)

引理4是非常关键的一个引理，完整的说明了idom、sdom的关系，也为工程实践铺好了道路。我们通过解释论文中的证明对引理4来进行说明，论文中说存在一个DFS生成树上的路径r *→idom(w) ∗→sdom(w)+→w，前面我们证明了idom(w)到w在T上必有路径，为什么sdom(w)到w也必有路径呢？下图是解释论文的证明思路
...
sdom(w)定义就是所有半支配候选节点中最小的。假设sdom(w) > parent(w)，那么sdom(w)不是候选半支配节点中最小的，parent(w)是最小的了，和定义就矛盾了，其实就是sdom要尽量小。那么得出sdom(w) < w后，根据引理1，sdom(w)到w的任意路径上经过一个T上的共同祖先v。那从sdom(w)到w的半支配路径也必须要经过v。这里就是一个典型的视角切换，图上的定理肯定能用在半支配路径这种特殊的路径上。

这个共同祖先v有可能在半支配路径内部吗？那么根据半支配路径的定义，任何内部节点都要大于w，而v是祖先应该小于w，因此v不可能在半支配路径的内部(不在内部说明是起点本身)，那么自然得出了sdom(w)是w在树上的祖先。重要的是视角要放在半支配路径上。

然而sdom和idom的关系是怎样的呢？我们考虑这么一条路径，r *→ sdom(w) 到w，sdom(w)到w的半支配路径内部节点都是大于w的，idom(w)不可能在这里，那么又已知任意路径上都包含idom(w)，那么idom(w)只可能在r *→sdom(w)的树路径上，也就是命题得证。

通过上面论证，隐约有一种以大见小的感觉，就是可以利用比较强的引理来专注于研究sdom(w)、idom(w)、w在树上的关系，而不要去关心图上的关系。半支配节点想在生成树T上找一个最早可能对w节点有影响的节点，通过定义本身入手去把不在树上的情况排除掉。前面我们证明半支配路径的祖先是sdom(w)时就会发现，通过这样定义会完美满足引理1。通过下面这幅图就容易理解了，半支配路径内部就是右树，右树和w没有什么关联，但是右树的父节点，一定也是左树的父节点，相当于找到了左树内部w在树上的祖先，祖先是有可能的支配节点。

![image-20250708102723468](./assets/image-20250708102723468.png)

勿在浮沙筑高台，定理再简单也得自己尝试想一想，循序渐进，看别人证明时，哪怕证明中只有一句似是而非的地方，都说明自己还没有真理解。如果不搞清楚困惑，后面某些步骤就会被困惑所桎梏，长此以往，困惑累积的越来越多，学习也难以为继。相反，如果我们用自己的方式理解了困惑，这会成为一种动力。

### 6.1.4 sdom的计算(定理4的用处）

前面的过程我们知道了sdom是什么，sdom有什么性质，那关键是怎么更方便地计算出来sdom呢？原论文中提出了一个定理4。这个定理4是说有一个节点u，u节点是大于w的，然后有一个w的前驱v，并且有树上路径u *→ v，如果存在这样的u，那么sdom(w)=sdom(u)。

![image-20250715162205882](./assets/image-20250715162205882.png)

我们先不看证明，先想一想这回事，就是u＞w，u又有树上的路径到v，那v肯定是大于w的，很像是一条半支配路径。

解释论文中给出了一种方法，证明很有意思，还是介绍一下。半支配路径的起点sdom只可能有两种情况，一个是不含内部节点的半支配路径，也就是w节点的前驱节点，记作sd1；另一种是有内部节点的半支配路径，记作sd2。sd1肯定是存在的，最少有parent(w)，而sd2不一定存在，不管存不存在先记为无穷，那么sdom肯定是min(sd1，sd2)
...
<<<<<<< HEAD
然而原论文的定理4，人家说的是u *→ v，然后v和w在图上有一条边，也就是完全脱离了半支配路径，直接是树边和非树边，我们上面的证明只能用在半支配路径里面。如何增强到原论文这个结论呢？可以这样理解，考虑**任意有内部节点的半支配路径**，有个内部节点u，v是倒数第二个节点，并且u *→v。如果内部节点u不是最小的，比它小的节点只能在它前面，因为后面是树路径，递增的；那么假设前面这个更小的节点是m，由于引理1，m＜u，m到u要经过一个树上的公共祖先g节点，即 m =V0, V1 ... g .. Vk = u ，而g节点是u在树上的祖先，就有一条树路径g *→ u，而u *→v进而得出 g *→v，而g是树上祖先，应该小于等于m，m又得最小，g只能是最小的，而g *→ v，说明g只是形式上的u。也就是说，任意半支配路径上，存在树路径u *→v和u是最小节点本质上是一回事。

上面说的是任意半支配路径，论文中的路径起点u本身就是大于w的，也可以构成半支配路径的内部节点，而且u明显是内部节点最小的，那么就有sd2(w) = sd(u)
...
>>>>>>> 216bcb31593b299b088a960b9846ca5f4d2c6025

![image-20250715140515542](./assets/image-20250715140515542.png)

在求DFS生成树时，可以标记树边，不需要再找半支配路径比大小了，因此原论文这样表述对工程更友好。然后就是通过半支配节点计算直接支配节点了。
