...
然而原论文的定理4，人家说的是u *→ v，然后v和w在图上有一条边，也就是完全脱离了半支配路径，直接是树边和非树边，我们上面的证明只能用在半支配路径里面。如何进行增强呢？u有G上的路径p到达w，令v是w的前驱，则v是p上的倒数第二个节点；令q是u到v的路径。u是最小的，根据引理1，说明q中有一个树上的公共祖先，而u是最小的，因此公共祖先不能在内部，只能是树上路径u *→v，就是要证明的命题。

![image-20250715140515542](./assets/image-20250715140515542.png)

原论文这样的表述更好一些，因为路径更容易被标记，不需要再找半支配路径比大小了。然后就是通过半支配节点计算直接支配节点了。
